...
void renderer3d::destroy_render()
{
...
void renderer3d::update_display_size()
{
...
void renderer3d::set_flags(renderer_flags f)
{
    flags_ = f;
}

font* renderer3d::add_font(const font_cfg& cfg)
{
#if defined(_DEBUG)
...
void renderer3d::remove_font(font* font)
{
#if defined(_DEBUG)
    assert_render_thread();
#endif
...
bool renderer3d::is_initialized()
{
    return is_initialized_;
}

void renderer3d::update_fonts_on_frame()
{
#if defined(_DEBUG)
    assert_render_thread();
#endif
...
void renderer3d::setup_render_state()
{
#if defined(_DEBUG)
    assert_render_thread();
#endif
...
void renderer3d::backup_render_state()
{
    context_->backup_render_state();
}

void renderer3d::restore_render_state()
{
    context_->restore_render_state();
}

void renderer3d::reset_render_data()
{
#if defined(_DEBUG)
    assert_render_thread();
#endif
...
void renderer3d::render()
{
    ...
    render_data_->vertex_buffer->update(
        vertices_.data(),
        vertices_.size() * sizeof(vertex3d)
    );
...
void renderer3d::set_multisampled(bool multisample)
{
    multisample ?
...
void renderer3d::ensure_capacity(std::uint32_t num_indices, std::uint32_t num_vertices)
{
    ...
    if (render_data_->vertex_count < num_vertices ||
        ...
        d.dynamic = true;
        d.size_bytes = count * sizeof(vertex3d);
        d.vb_stride = sizeof(vertex3d);
...
void renderer3d::font_update_thread()
{
...
texture_handle renderer3d::font_texture() const noexcept
{
    assert(render_data_);
    assert(render_data_->font_view);
