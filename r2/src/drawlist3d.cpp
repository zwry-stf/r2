...
void renderer3d::add_convex_filled(const path_point_t* points, std::uint32_t num_points, color_u32 col)
{
    if (num_points < 3u ||
    ...
    if (flags_.anti_aliased_fill) {
        ...
        temp_buffer3d_.clear();
        temp_buffer3d_.resize(num_points);
        for (std::uint32_t i0 = num_points - 1u, i1 = 0u; i1 < num_points; i0 = i1++) {
            ...
            temp_buffer3d_[i0].pos.x = d.y;
            temp_buffer3d_[i0].pos.y = -d.x;
        }
        const color_u32 col_no_alpha = col & ~color::alpha_mask;
        for (std::uint32_t i0 = num_points - 1u, i1 = 0u; i1 < num_points; i0 = i1++) {
            const vec2& n0 = temp_buffer3d_[i0].pos;
            const vec2& n1 = temp_buffer3d_[i1].pos;
...
void renderer3d::add_shadow_convex(const path_point_t* points, std::uint32_t num_points, color_u32 col, float shadow_size, bool filled)
{
    if (num_points < 3u ||
    ...
    for (std::uint32_t edge_index = 0u; edge_index < num_edges; edge_index++) {
        ...
        if (cos_angle_coverage < 0.999999f) {
            ...
            for (std::uint32_t step = 0u; step < num_steps; step++) {
                ...
                vertices_.emplace_back(edge_start, edge_start_depth, solid_uv, col);
                vertices_.emplace_back(outer_edge_end, edge_start_depth, expanded_edge_uv, col);
                vertices_.emplace_back(outer_edge_start, edge_start_depth, other_edge_uv, col);
        ...
        if (edge_length > 0.00001f) {
            ...
            vertices_.emplace_back(edge_start, edge_start_depth, solid_uv, col);
            vertices_.emplace_back(edge_end, edge_end_depth, solid_uv, col);
            vertices_.emplace_back(outer_edge_end, edge_end_depth, scaled_edge_uv_end, col);
            vertices_.emplace_back(outer_edge_start, edge_start_depth, scaled_edge_uv_start, col);
            indices_.emplace_back(vertex_ptr_ + 0u);
            indices_.emplace_back(vertex_ptr_ + 1u);
            indices_.emplace_back(vertex_ptr_ + 2u);
            indices_.emplace_back(vertex_ptr_ + 0u);
...
void renderer3d::add_lines(const path_point_t* points, std::uint32_t num_points, color_u32 col, float line_width, bool closed)
{
    if (num_points < 2u ||
        (col & color::alpha_mask) == 0u) [[unlikely]]
        return;
    ...
    [[likely]] if (flags_.anti_aliased_lines) {
        ...
        if (use_texture || !thick_line) {
            ...
            if (use_texture) {
                ...
                for (std::uint32_t i = 0u; i < num_points; i++) {
                    vertices_.emplace_back(temp_points[i * 2u + 0u], points[i].depth, tex_uv0, col);
                    vertices_.emplace_back(temp_points[i * 2u + 1u], points[i].depth, tex_uv1, col);
                    vertex_ptr_ += 2u;
                }
            }
            else {
                for (std::uint32_t i = 0u; i < num_points; i++) {
                    vertices_.emplace_back(points[i].pos, points[i].depth, opaque_uv, col);
                    vertices_.emplace_back(temp_points[i * 2u + 0u], points[i].depth, opaque_uv, col_no_alpha);
                    vertices_.emplace_back(temp_points[i * 2u + 1u], points[i].depth, opaque_uv, col_no_alpha);
                    vertex_ptr_ += 3u;
                }
            }
        }
            ...
            if (!closed) {
                ...
                temp_points[0] = points[0].pos + temp_normals[0] * vec2(half_inner_line_width + aa_scale_);
                ...
                temp_points[3] = points[0].pos - temp_normals[0] * vec2(half_inner_line_width + aa_scale_);
                temp_points[points_last * 4u + 0u] = points[points_last].pos + temp_normals[points_last] * vec2(half_inner_line_width + aa_scale_);
                ...
                temp_points[points_last * 4u + 3u] = points[points_last].pos - temp_normals[points_last] * vec2(half_inner_line_width + aa_scale_);
            }
            std::uint32_t idx1 = vertex_ptr_;
            for (std::uint32_t i1 = 0u; i1 < count; i1++) {
                const std::uint32_t i2 = (i1 + 1u) == num_points ? 0u : (i1 + 1u);
            ...
            for (std::uint32_t i = 0; i < num_points; i++) {
                vertices_.emplace_back(temp_points[i * 4u + 0u], points[i].depth, opaque_uv, col_no_alpha);
                vertices_.emplace_back(temp_points[i * 4u + 1u], points[i].depth, opaque_uv, col);
                vertices_.emplace_back(temp_points[i * 4u + 2u], points[i].depth, opaque_uv, col);
                vertices_.emplace_back(temp_points[i * 4u + 3u], points[i].depth, opaque_uv, col_no_alpha);
                vertex_ptr_ += 4u;
            }
        }
    }
        ...
        for (std::uint32_t i1 = 0u; i1 < count; i1++) {
            ...
            vertices_.emplace_back(vec2{ p1.x + d.y, p1.y - d.x }, points[i1].depth, opaque_uv, col);
            vertices_.emplace_back(vec2{ p2.x + d.y, p2.y - d.x }, points[i2].depth, opaque_uv, col);
            vertices_.emplace_back(vec2{ p2.x - d.y, p2.y + d.x }, points[i2].depth, opaque_uv, col);
            vertices_.emplace_back(vec2{ p1.x - d.y, p1.y + d.x }, points[i1].depth, opaque_uv, col);
            indices_.emplace_back(vertex_ptr_ + 0u);
            indices_.emplace_back(vertex_ptr_ + 1u);
...
void renderer3d::add_line_multicolor(const path_point_t& start_p, const path_point_t& end_p, color_u32 col_start, color_u32 col_end, float line_width)
{
    ...
    if (!flags_.anti_aliased_lines) [[unlikely]] {
        ...
        vertices_.emplace_back(start - n, start_p.depth, opaque_uv, col_start);
        vertices_.emplace_back(start + n, start_p.depth, opaque_uv, col_start);
        vertices_.emplace_back(end + n, end_p.depth, opaque_uv, col_end);
        vertices_.emplace_back(end - n, end_p.depth, opaque_uv, col_end);
        vertex_ptr_ += 4u;
        return;
    }
    ...
    if (use_texture || !thick_line) {
        ...
        if (use_texture) {
            ...
            const vec2 tex_uv0(tex_uvs.x, tex_uvs.y);
            const vec2 tex_uv1(tex_uvs.z, tex_uvs.w);
            vertices_.emplace_back(temp_points[0], start_p.depth, tex_uv0, col_start);
            vertices_.emplace_back(temp_points[1], start_p.depth, tex_uv1, col_start);
            vertices_.emplace_back(temp_points[2], end_p.depth, tex_uv0, col_end);
            vertices_.emplace_back(temp_points[3], end_p.depth, tex_uv1, col_end);
            vertex_ptr_ += 4u;
        }
        else {
            vertices_.emplace_back(start, start_p.depth, opaque_uv, col_start);
            vertices_.emplace_back(temp_points[0], start_p.depth, opaque_uv, col_no_alpha_start);
            vertices_.emplace_back(temp_points[1], start_p.depth, opaque_uv, col_no_alpha_start);
            vertices_.emplace_back(end, end_p.depth, opaque_uv, col_end);
            vertices_.emplace_back(temp_points[2], end_p.depth, opaque_uv, col_no_alpha_end);
            vertices_.emplace_back(temp_points[3], end_p.depth, opaque_uv, col_no_alpha_end);
            vertex_ptr_ += 6u;
        }
    }
    else {
        ...
        indices_.emplace_back(idx2 + 2u);
        vertices_.emplace_back(temp_points[0], start_p.depth, opaque_uv, col_no_alpha_start);
        vertices_.emplace_back(temp_points[1], start_p.depth, opaque_uv, col_start);
        vertices_.emplace_back(temp_points[2], start_p.depth, opaque_uv, col_start);
        vertices_.emplace_back(temp_points[3], start_p.depth, opaque_uv, col_no_alpha_start);
        vertices_.emplace_back(temp_points[4], end_p.depth, opaque_uv, col_no_alpha_end);
        vertices_.emplace_back(temp_points[5], end_p.depth, opaque_uv, col_end);
        vertices_.emplace_back(temp_points[6], end_p.depth, opaque_uv, col_end);
        vertices_.emplace_back(temp_points[7], end_p.depth, opaque_uv, col_no_alpha_end);
        vertex_ptr_ += 8u;
    }
}
r2_end_