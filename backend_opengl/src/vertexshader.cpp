...
gl_vertexshader::gl_vertexshader(gl_context* ctx, gl_compiled_shader* shader_data)
    : r2::vertexshader(),
      gl_object(ctx)
{
    ...
    if (shader == 0u ||
        ...
        set_error(
            std::to_underlying(gl_vertexshader_error::shader_generation)
        );
        return;
    }
    ...
    if (gl_err != GL_NO_ERROR) {
        set_error(
            std::to_underlying(gl_vertexshader_error::shader_compilation),
